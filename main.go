...
func runRestApi(*cli.Context) error{
	...
	//homework
	homeworkrepo,err:=homework.NewLessonRepository(conf)
	if err!=nil{
		return err
	}
	homeworkinter:=homework.NewHomeWorkInternship(homeworkrepo)
	homeworkendpoints:=homework.NewEndpointsFactory(homeworkinter)
	router.Methods("POST").Path("/homework/").HandlerFunc(homeworkendpoints.AddHomework())
	router.Methods("GET").Path("/homework/").HandlerFunc(homeworkendpoints.GetHomeWork())
	//lessons
	lessonsrepo,err:=lessons.NewLessonRepository(conf)
	if err!=nil{
		return err
	}
	lessonsendpoints:=lessons.NewEndpointsFactory(lessonsrepo,attendancerepo,materialrepo)
	...
	router.Methods("GET").Path("/lessons/{id}/attendances/").HandlerFunc(lessonsendpoints.GetAttendance("id"))
	router.Methods("GET").Path("/lessons/{id}/material/").HandlerFunc(lessonsendpoints.GetMaterial("id"))
	//schedule
	schedulerepo,err:=schedule.NewScheduleRepository(conf)
	if err!=nil{
		return err
	}
	scheduleendpoints:=schedule.NewEndpointsFactory(schedulerepo,lessonsrepo)
	...
	router.Methods("GET").Path("/schedule/{id}/lessons/").HandlerFunc(scheduleendpoints.GetLessons("id"))
	...
	studentendpoints:=students.NewEndpointsFactory(studentrepo,redisconnect)
	...
	coursesendpoints:=courses.NewEndpointsFactory(coursesrepo,lessonsrepo,schedulerepo,studentrepo)
	...
	router.Methods("GET").Path("/courses/{id}/schedule/").HandlerFunc(coursesendpoints.GetScheduleByCourse("id"))
	router.Methods("GET").Path("/courses/{id}/students/").HandlerFunc(coursesendpoints.GetStudentsByCourse("id"))
	//
	...
	router.Methods("POST").Path("/teachers/courses/").HandlerFunc(teachersendpoints.GetTeacherCourses())
